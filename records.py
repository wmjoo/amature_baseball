...
with tab_sn_teams: # 팀 기록 탭
    ...
    with tab_sn_teams_team: # 선택 팀 기록 탭
        ...
        mainteam_statrank_h = pd.concat([df1_h, df2_h], axis = 0).rename(columns = hitter_data_EnKr, inplace=False).set_index('공격지표').T
        st.write(mainteam_statrank_h['기록'])
        st.write(mainteam_statrank_h['순위'])
        def format_cell(x):
