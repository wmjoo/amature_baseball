...
team_name_dict = team_name_dict_2025miB #| team_name_dict_2025miB
...
hitter_data_EnKr = {'Name': '성명', 'No': '배번', 'AVG': '타율', 'G': '경기', 'PA': '타석', 'AB': '타수', 'R': '득점', 
                    ...
                    'SB': '도루', 'CS': '도실', 'SH': '희타', 'SF': '희비', 'BB': '볼넷', 'IBB': '고의4구', 'HBP': '사구', 
                    'SO': '삼진', 'DP': '병살', 'SLG': '장타율', 'OBP': '출루율', 'SB%': '도루성공률', 'MHit': '멀티히트', 'OPS': 'OPS', 
                    'BB/K': 'BB/K', 'XBH/H': '장타/안타', 'Team': '팀'}
...
with top_col1:
    this_year = 2025
    # 일정표 URL 설정
    schd_url = f"http://www.gameone.kr/club/info/schedule/table?club_idx=4653&kind=&season={this_year}"
    # HTTP GET 요청
    ...
    next_game_teamname = ((next_game['선공'] + next_game['후공']).str.replace('SKCC Wings', ''))[0]
    # 선공/후공 팀명에 스타일 적용
        ...
        if user_password_aireport == st.secrets["password_update"]: # Correct Password
            ...
            if GOOGLE_API_KEY:
                ...
                if (df_season is not None) & (df_season_p is not None):
                    ...
                    그리고 사회인야구에서 홈런을 기록하는 것은 매우 어렵기 떄문에 통산 홈런이 있다면 해당 내용을 언급해주세요 (특히 홈런 숫자를 언급할 떄는 틀리지 않도록 신중하게 생각하고 말해주세요).
